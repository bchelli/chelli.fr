...
$(window).load(function () {
	...
	$container.css({
		'width': $panels[0].offsetWidth * $panels.length,
	...
	function selectNav() {
		...
		$container.stop().animate({
			'height':$('#'+$(this).attr('href').substr(1)).outerHeight(true)
		}, 'fast', 'linear', function(){
			var mt = $('#slider').css('margin-top');
			$('#slider').css({
				'margin-top':0,
				'margin-bottom':0
			});
			var m = Math.floor(($('#wrapper').outerHeight()-$('#header').outerHeight()-$('#footer').outerHeight()-$('#slider').outerHeight()-20)/2);
			$('#slider').css({
				'margin-top':mt,
				'margin-bottom':mt
			}).animate({
